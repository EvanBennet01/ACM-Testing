...
#define fox_init_dir 'u'						// fox starts moving up
#define rabbit_init_x (width/3*1)
#define rabbit_init_y (height/2)
#define rabbit_init_dir 'd'						// rabbit starts moving down
...
// which is the target pointed at by the pointer
    case 'd':
        if (*y_ptr < 18) (*y_ptr)++; // height
        break;
    case 'l':
        if (*x_ptr > 1) (*x_ptr)--;
        break;
    case 'r':
        if (*x_ptr < 40) (*x_ptr)++; //width
        break;
    }
}
...
int main ()
{
    ...
    while (1)
    {
        ...
        int n = rand() % 4; // generate number at random to modulo 4
        switch (n)			// select rabbit direction relating to random number
        {
        case 0:
            rabbit_dir = 'u';
            break;
        ...
        update_coord (&rabbit_x, &rabbit_y, rabbit_dir);	// n coordinates 
        printf ("\033[2J\033[%d;%dH@\033[%d;%dH*", fox_y, fox_x, rabbit_y, rabbit_x); // \033 = clear therefore 31-9 = 22 characters
        fflush (stdout);

        if ( (fox_y == rabbit_y) && (fox_x == rabbit_x) ) break; // game termination
    }
